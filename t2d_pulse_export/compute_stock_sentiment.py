...
# 2) Ensure the stock_sentiment_history table exists (with raw_sentiment_score)
...
# 3) Fetch the previous day's raw_sentiment_score for each ticker, if available
try:
    prev_df = pd.read_sql(
        """
...
# 4) Get the latest market-cap and prior-day market-cap for each ticker
mcap_df = pd.read_sql(
    """
    WITH mc AS (
...
# 6) Update raw sentiment with 20-day EMA formula: EMA_new = α*R_new + (1–α)*EMA_prev
alpha = 2 / (20 + 1)
...
# 7) Normalize to 0–100: percent form + 50 shift
mcap_df['sentiment_score'] = mcap_df['raw_sentiment_score'] * 100 + 50

# 8) Persist only the new day's sentiment into stock_sentiment_history
with engine.begin() as conn:
    insert_sql = text(

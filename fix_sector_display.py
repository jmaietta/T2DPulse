...
def load_sector_market_caps_from_csv():
    ...
    # Try different file paths, prioritizing the newer file generated by sector_market_cap.py
    file_paths = [
        'sector_market_caps.csv',  # New file from sector_market_cap.py (preferred)
        ...
        if os.path.exists(file_path):
            ...
            try:
                ...
                if 'date' not in df.columns:
                    logger.warning(f"Date column missing in {file_path}")
                    continue
                    
                # Convert date column to datetime
                df['date'] = pd.to_datetime(df['date'])
                
                # Handle different formats in different files
                if file_path == 'sector_market_caps.csv':
                    # The new file has 'sector' and 'market_cap' columns
                    # We need to pivot this to get sectors as columns
                    if 'sector' in df.columns and 'market_cap' in df.columns:
                        # Get the latest date from the file to use most recent data
                        latest_date = df['date'].max()
                        recent_df = df[df['date'] >= (latest_date - timedelta(days=30))]
                        # Pivot to get sectors as columns
                        pivoted_df = recent_df.pivot(index='date', columns='sector', values='market_cap').reset_index()
                        return pivoted_df
                    else:
                        logger.warning(f"Expected columns missing in {file_path}")
                        continue
                else:
                    # Original format - no transformation needed
                    return df

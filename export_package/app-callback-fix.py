"""
T2D Pulse Dashboard - Direct Weight Input Fix

This file contains only the callback function that handles direct weight input changes
using the "Apply" button. This is the most important part to debug.
"""
...
def update_weight_from_input(n_clicks_list, input_values, input_ids, weights_json):
    ...
    Args:
        n_clicks_list: List of click counts for all Apply buttons
        input_values: List of values from all weight input fields
        input_ids: List of IDs for all weight input fields (contains sector names)
        weights_json: Current weights as JSON string
        
    Returns:
        str: Updated weights as JSON string
    """
    ...
    # Print detailed trigger information for debugging
    ...
    try:
        ...
        # Update global weights (needed for persistence between callbacks)
        global sector_weights
        sector_weights = weights.copy()
        
        # Return weights as JSON string
...
def update_weight_inputs(weights_json):
    ...
    Args:
        weights_json: Sector weights as JSON string
        
    Returns:
        tuple: Updated values for weight inputs, T2D Pulse score, button styles,
              notification message, and notification style
    """
    # Use global weights if no JSON is provided
    if not weights_json:
        global sector_weights
        weights = sector_weights
    else:
    ...
    print(f"Weight values for display: {weight_values}")
    ...
    try:
        ...
        print(f"Calculated new T2D Pulse score: {t2d_pulse_display}")
    except Exception as e:
    ...
    for _ in range(len(weight_values)):
        button_styles.append({
            "fontSize": "12px",
            "padding": "4px 8px",
            "backgroundColor": "#e74c3c",  # Red highlight
            "color": "white",
            "border": "none",
            "borderRadius": "4px",
            "cursor": "pointer",
